	...
	switch (info.iProjectileType)
	{
		...
		case kProjLaser:
		{
			...
			EV_TraceLine(
				...
				PM_STUDIO_IGNORE, args->entindex, tr);

			EV_TracePop();

			client::efx::UserTracerParticle(
				gun,
				forward * 1500.0F,
				5.0F * tr.fraction,
				2,
		...
		case kProjFlame:
		{
			...
			if (client::PM_PointContents(gun, nullptr) == CONTENTS_EMPTY)
			{
				auto flame = client::efx::TempSprite(
					...
					1.0F,
					1.0F,
					FTENT_SPRANIMATE | FTENT_PERSIST | FTENT_CLIENTCUSTOM | FTENT_COLLIDEKILL | FTENT_COLLIDENONCLIENTS);
