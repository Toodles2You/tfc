...
int GetWeaponData(struct edict_s* player, struct weapon_data_s* info)
{
	...
	for (auto gun : pl->m_lpPlayerWeapons)
	{
		...
		*(int*)&data->m_flTimeWeaponIdle = V_max(gun->m_iTimeWeaponIdle, -1);
		*(int*)&data->m_flNextPrimaryAttack = V_max(gun->m_iNextPrimaryAttack, -1);
		*(int*)&data->m_flNextSecondaryAttack = V_max(gun->m_iNextSecondaryAttack, -1);
		data->m_fInReload = static_cast<int>(gun->m_fInReload);
		data->m_fInSpecialReload = gun->m_fInSpecialReload;
		data->fuser1 = V_max(gun->pev->fuser1, -0.001);
